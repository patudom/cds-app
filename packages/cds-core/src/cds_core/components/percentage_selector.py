...
def PercentageSelector(viewers: List[Viewer],
    ...
    def _model_factory(option):
        return solara.lab.computed(lambda opt=option: selected.value == opt)

    from ..components import InfoDialog
            ...
            for option in options:
                ...
                with solara.Row(style={"align-items": "center"}, classes=["switch-panel"]):
                    solara.Switch(value=model,
                                  label=f"{option}%",
                                  on_value=lambda value, opt=option: _update_selected(opt, value))
                    InfoDialog(
                        dialog=False,
