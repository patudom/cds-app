...
def create_example_subsets(gjapp: JupyterApplication, example_data: Data):
    ...
    if "first measurement" not in (s.label for s in current_subsets):
        first = example_data.new_subset(
            example_data.id["measurement_number"] == "first", label="first measurement"
        )
        first.style.color = MY_DATA_COLOR
        first.style.alpha = 1.0
    if "second measurement" not in (s.label for s in current_subsets):
        second = example_data.new_subset(
            example_data.id["measurement_number"] == "second",
...
def link_example_seed_and_measurements(gjapp: JupyterApplication):
    if (
        EXAMPLE_GALAXY_SEED_DATA in gjapp.data_collection
...
def load_and_create_seed_data(
    ...
    first = Data(
        ...
        **{
            k: np.asarray(
                [r[k] for r in example_seed_data if r["measurement_number"] == "first"]
            )
            for k in example_seed_data[0].keys()
        },
    )
    ...
    second = Data(
        ...
        **{
            k: np.asarray(
                [r[k] for r in example_seed_data if r["measurement_number"] == "second"]
            )
            for k in example_seed_data[0].keys()
        },
    )
    ...
    tutorial = Data(
        label=EXAMPLE_GALAXY_SEED_DATA + "_tutorial",
        **{
