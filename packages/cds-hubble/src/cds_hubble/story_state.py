...
from cds_core.base_states import BaseStoryState, register_model
...
@register_model("story", "hubbles_law")
class LocalState(BaseStoryState):
...
def get_free_response(
    ...
    new = dict(
        tag=tag, response="", initialized=True, stage=component_state.value.stage_id
    )
...
def fix_free_responses_stage_missing(
    tag,
    local_state: Reactive[LocalState],
    component_state: Reactive[BaseComponentStateT],
):
...
def mc_callback(
    ...
    # mc-score event returns a data which is an mc-score dictionary (includes tag)
        ...
        new_score = mc_scoring[
            event[1]["tag"]
        ].copy()  # make a copy of the current score
        if "stage" not in new_score:
            new_score["stage"] = component_state.value.stage_id
        new_score.update(
            event[1]
        )  # update with new score, choice, tries, and wrong_attempts, but keeps the stage
        ...
        try:
            ...
            logger.error(
                f"Invalid score value for tag:{event[1]['tag']}: {event[1]['score']}"
            )
            score = 0
